...
const C = {
  navy:'#1B3A6B', blue:'#2E5090', accent:'#4A90D9',
  gold:'#C9A84C', gold2:'#F0D98A',
  white:'#FFFFFF', offwhite:'#F7FAFD',
  light:'#EEF3FA', text:'#0F1B2D', sub:'#5A6B82', border:'#C8D8EE',
};
...
const TOOLS    = ['ChatGPT','Claude','Gemini','뤼튼(wrtn)','Microsoft Copilot','네이버 클로바X','기타 AI 도구','AI 미사용'];
const USES     = ['문서·보고서 작성','이메일·공문 초안','정보 검색·요약','번역','아이디어·기획','데이터 분석','이미지·영상 제작','코딩·자동화','기타'];
const LEVELS   = ['① 전혀 모름 / AI가 생소함','② 가끔 검색 수준으로만 사용','③ 문서 작성 등 기본 업무 활용','④ 여러 도구 조합해 적극 활용','⑤ AI 자동화·워크플로우 설계 가능'];
const WISHES   = ['문서·보고서 자동 작성','환자 안내·문자 자동 발송','데이터 분석 후 보고서 생성','법령·규정 즉시 검색·안내','회의록·메모 자동 정리','SNS·블로그·유튜브 콘텐츠 초안','이미지·영상 편집 보조','환자 예약·문의 24시간 대응','기타'];
const CONCERNS = ['개인정보·환자정보 유출 위험','AI 오정보 제공 우려','사용법 학습 부담','일자리 위협','기존 EMR 연동 어려움','AI 결과물 신뢰도 (재확인 필요)','특별한 우려 없음','기타'];
const EDU      = ['완전 초보 (AI 기초부터)','기초 — ChatGPT·Claude 활용법','중급 — 부서 특화 프롬프트 작성','고급 — AI 자동화 워크플로우 설계','관리자용 — AI 도입 전략 이해'];

const emptyForm = () => ({
  dept:'', deptOther:'', name:'', position:'', email:'',
  q1:[], q1other:'', q2:[], q3:'',
    ...
    <style>{`
      ...
      *{box-sizing:border-box;margin:0;padding:0;}
      ...
      .btn-gold{background:#C9A84C;color:#1B3A6B;font-family:'Noto Sans KR',sans-serif;
        font-weight:700;font-size:15px;padding:13px 30px;border:none;border-radius:50px;cursor:pointer;transition:all .22s ease;}
      .btn-gold:hover{background:#F0D98A;transform:translateY(-2px);box-shadow:0 8px 22px rgba(201,168,76,.38);}
      .btn-gold:disabled{opacity:.4;cursor:not-allowed;transform:none;}
      .btn-navy{background:#1B3A6B;color:white;font-family:'Noto Sans KR',sans-serif;
        font-weight:600;font-size:14px;padding:12px 26px;border:none;border-radius:50px;cursor:pointer;transition:all .22s ease;}
      ...
      input,textarea,select{
        ...
        background:white;color:#0F1B2D;transition:border-color .18s ease;width:100%;outline:none;}
      input:focus,textarea:focus,select:focus{border-color:#C9A84C;}
      textarea{resize:vertical;line-height:1.7;}
      .card{background:white;border-radius:18px;padding:26px;border:1px solid #C8D8EE;margin-bottom:18px;}
      .card-dark{background:rgba(255,255,255,.05);border-radius:18px;padding:24px;border:1px solid rgba(255,255,255,.08);margin-bottom:16px;}
      .tag{display:inline-flex;align-items:center;background:#C9A84C;color:#1B3A6B;
        font-weight:700;font-size:11px;padding:3px 10px;border-radius:20px;letter-spacing:.5px;margin-right:8px;}
      .pill-btn{padding:8px 15px;border-radius:20px;cursor:pointer;font-family:'Noto Sans KR',sans-serif;
        font-size:13px;transition:all .18s ease;border:1.5px solid #C8D8EE;background:white;color:#5A6B82;}
    ...
    <div style={{minHeight:'100vh',background:'#0A1628',display:'flex',
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',
        ...
        <div className="fu" style={{marginBottom:10,textAlign:'center'}}>
          <div style={{display:'inline-flex',flexDirection:'column',alignItems:'center',
            ...
            <span style={{color:'white',fontWeight:900,fontSize:20,letterSpacing:2,marginBottom:3}}>에이스병원</span>
            <span style={{color:'#C9A84C',fontSize:11,letterSpacing:3,fontWeight:500}}>ACE HOSPITAL</span>
          </div>
        </div>
        <div className="fu2" style={{marginBottom:28,textAlign:'center'}}>
          <span style={{color:'rgba(255,255,255,.28)',fontSize:11,letterSpacing:2}}>AI 컨설팅 파트너 · SIMPLIT</span>
        </div>
        <div className="fu2" style={{textAlign:'center',maxWidth:600,marginBottom:14}}>
          <div style={{color:'#C9A84C',fontSize:11,fontWeight:700,letterSpacing:4,marginBottom:20,textTransform:'uppercase'}}>
            AI 도입 부서 업무 현황 파악 설문
          </div>
          <h1 style={{color:'white',fontSize:'clamp(24px,4.2vw,42px)',fontWeight:900,lineHeight:1.25,marginBottom:22}}>
            에이스병원이 더 스마트하게<br/>
        ...
        <div className="fu3" style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
          ...
          <span style={{color:'rgba(255,255,255,.3)',fontSize:12}}>응답 내용은 에이스병원 AI 도입 전략에만 활용됩니다</span>
        </div>
        <div className="fu4" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',
          gap:12,maxWidth:480,marginTop:48,width:'100%'}}>
          {[{icon:'📋',t:'5개 파트 · 11개 문항'},{icon:'⏱',t:'약 10~15분 소요'},{icon:'🔒',t:'내부 전략 수립 목적'}].map((c,i)=>(
            <div key={i} style={{background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.09)',
              borderRadius:14,padding:'16px 10px',textAlign:'center',backdropFilter:'blur(8px)'}}>
      ...
      <div style={{textAlign:'center',padding:'16px',position:'relative',zIndex:1}}>
        <button onClick={onAdmin} style={{background:'none',border:'none',
          color:'rgba(255,255,255,.1)',fontSize:11,cursor:'pointer',letterSpacing:2}}>관리자</button>
      </div>
    </div>
  );
}
      ...
      <div style={{maxWidth:720,margin:'0 auto'}}>
        ...
        <div style={{paddingBottom:8}}>
          <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'rgba(255,255,255,.45)',marginBottom:4}}>
            <span>{titles[step-1]}</span><span>{Math.round(pct)}%</span>
          </div>
          <div style={{height:3,background:'rgba(255,255,255,.14)',borderRadius:2}}>
            <div style={{height:'100%',background:'#C9A84C',borderRadius:2,width:`${pct}%`,transition:'width .4s ease'}}/>
          </div>
        </div>
        <div style={{display:'flex',gap:5,padding:'7px 0',overflowX:'auto',scrollbarWidth:'none'}}>
          {titles.map((t,i)=>(
            <div key={i} style={{fontSize:10,padding:'3px 10px',borderRadius:20,whiteSpace:'nowrap',flexShrink:0,
...
function Step1({ form, setForm, toggleCheck }) {
  return (
    <div>
      ...
      <div className="card">
        <h3 style={{fontSize:14,fontWeight:700,color:'#1B3A6B',marginBottom:16,paddingBottom:10,borderBottom:'2px solid #EEF3FA'}}>📌 기본 정보</h3>
        <div style={{marginBottom:14}}>
          <label style={{fontSize:12,fontWeight:700,color:'#5A6B82',display:'block',marginBottom:6}}>소속 부서 *</label>
          ...
          {form.dept==='기타'&&(
            <input type="text" placeholder="부서명을 직접 입력해 주세요"
              value={form.deptOther} onChange={e=>setForm(f=>({...f,deptOther:e.target.value}))} style={{marginTop:8}}/>
          )}
        </div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
          <div>
            <label style={{fontSize:12,fontWeight:700,color:'#5A6B82',display:'block',marginBottom:6}}>성명 *</label>
            <input type="text" placeholder="홍길동"
              value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/>
          </div>
          <div>
            <label style={{fontSize:12,fontWeight:700,color:'#5A6B82',display:'block',marginBottom:6}}>직책 *</label>
            <input type="text" placeholder="수간호사 / 팀장 / 주임 등"
              value={form.position} onChange={e=>setForm(f=>({...f,position:e.target.value}))}/>
          </div>
        </div>
        <div>
          <label style={{fontSize:12,fontWeight:700,color:'#5A6B82',display:'block',marginBottom:6}}>이메일 주소</label>
          <input type="email" placeholder="example@acehospital.co.kr"
            value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))}/>
          <p style={{fontSize:11,color:'#5A6B82',marginTop:5}}>분석 결과 공유 목적으로만 사용됩니다. 선택 입력입니다.</p>
        </div>
      </div>
      <div className="card">
        <QHead n={1} title="현재 사용 중인 AI 도구를 모두 선택해 주세요." hint="복수 선택 가능"/>
        {TOOLS.map(t=><CB key={t} label={t} checked={form.q1.includes(t)} onChange={()=>toggleCheck('q1',t)}/>)}
        {form.q1.includes('기타 AI 도구')&&(
          <input type="text" placeholder="사용 중인 AI 도구명을 입력해 주세요"
            value={form.q1other} onChange={e=>setForm(f=>({...f,q1other:e.target.value}))} style={{marginTop:8}}/>
        )}
      </div>
      <div className="card">
        <QHead n={2} title="AI를 주로 어떤 용도로 사용하시나요?" hint="복수 선택 가능"/>
        {USES.map(u=><CB key={u} label={u} checked={form.q2.includes(u)} onChange={()=>toggleCheck('q2',u)}/>)}
      </div>
      <div className="card">
        <QHead n={3} title="현재 AI 활용 수준을 스스로 평가해 주세요."/>
        {LEVELS.map(l=><RB key={l} label={l} selected={form.q3===l} onChange={()=>setForm(f=>({...f,q3:l}))}/>)}
      </div>
    </div>
  );
}
...
function Step2({ form, setForm }) {
  return (
    <div>
      ...
      <div className="card">
        <QHead n={4} title="자주 반복되는 업무를 기재해 주세요." hint="빈도 예시: 매일/주1회/월1회 | AI 희망도: 1(낮음)~5(높음)"/>
        <div style={{display:'grid',gridTemplateColumns:'2.5fr 1fr 1fr .9fr',gap:7,marginBottom:9}}>
          {['업무명','빈도','소요시간','AI희망도'].map(h=>(
            <div key={h} style={{fontSize:11,fontWeight:700,color:'white',background:'#1B3A6B',padding:'7px 9px',borderRadius:7,textAlign:'center'}}>{h}</div>
          ))}
        </div>
        {form.q4.map((row,i)=>(
          <div key={i} style={{display:'grid',gridTemplateColumns:'2.5fr 1fr 1fr .9fr',gap:7,marginBottom:7,alignItems:'center'}}>
            <input type="text" placeholder={`업무 ${i+1}`} value={row.task}
              onChange={e=>{const q4=[...form.q4];q4[i]={...q4[i],task:e.target.value};setForm(f=>({...f,q4}));}} style={{fontSize:13}}/>
            <input type="text" placeholder="매일" value={row.freq}
              onChange={e=>{const q4=[...form.q4];q4[i]={...q4[i],freq:e.target.value};setForm(f=>({...f,q4}));}} style={{fontSize:13,textAlign:'center'}}/>
            <input type="text" placeholder="30분" value={row.time}
              onChange={e=>{const q4=[...form.q4];q4[i]={...q4[i],time:e.target.value};setForm(f=>({...f,q4}));}} style={{fontSize:13,textAlign:'center'}}/>
      ...
      <div className="card">
        ...
        {[1,2,3].map(n=>(
          <div key={n} style={{display:'flex',gap:10,alignItems:'center',marginBottom:10}}>
            <div style={{width:30,height:30,borderRadius:7,flexShrink:0,fontWeight:800,fontSize:13,
              display:'flex',alignItems:'center',justifyContent:'center',color:'#1B3A6B',
              background:n===1?'#C9A84C':n===2?'#B8CCEE':'#CCBFA0'}}>{n}</div>
            <input type="text" placeholder={`${n}순위 업무`} value={form.q5[n-1]}
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        {WISHES.map(w=><CB key={w} label={w} checked={form.q7.includes(w)} onChange={()=>toggleCheck('q7',w)}/>)}
        {form.q7.includes('기타')&&(
      ...
      <div className="card">
        ...
        {CONCERNS.map(c=><CB key={c} label={c} checked={form.q8.includes(c)} onChange={()=>toggleCheck('q8',c)}/>)}
        {form.q8.includes('기타')&&(
  ...
  return (
    <div>
      ...
      <div className="card">
        ...
        {EDU.map(e=><RB key={e} label={e} selected={form.q9===e} onChange={()=>setForm(f=>({...f,q9:e}))}/>)}
      </div>
      <div className="card">
...
function Step5({ form, setForm }) {
  return (
    <div>
      ...
      <div style={{background:'#EEF3FA',borderRadius:14,padding:18,border:'1px solid #C8D8EE',display:'flex',gap:14,alignItems:'flex-start'}}>
...
// ── ✅ FormView — App 바깥에 독립적으로 정의 (한글 입력 문제 해결 핵심) ────
function FormView({ step, total, form, setForm, toggleCheck, onNext, onPrev }) {
  return (
...
function ThanksView({ onNew }) {
  return (
    <div style={{minHeight:'100vh',background:'#0A1628',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      <GlobalStyle/>
      <div className="si" style={{background:'rgba(255,255,255,.06)',border:'1px solid rgba(201,168,76,.28)',
        borderRadius:26,padding:'52px 44px',textAlign:'center',maxWidth:460,width:'100%',backdropFilter:'blur(18px)'}}>
        <div style={{width:64,height:64,borderRadius:'50%',background:'#C9A84C',
          display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 24px',fontSize:28}}>✓</div>
        <h2 style={{color:'white',fontSize:22,fontWeight:800,marginBottom:12}}>제출이 완료됐습니다!</h2>
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0A1628',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      ...
      <div style={{background:'rgba(255,255,255,.06)',borderRadius:22,padding:38,
        border:'1px solid rgba(255,255,255,.09)',backdropFilter:'blur(18px)',width:'100%',maxWidth:340,textAlign:'center'}}>
        ...
        <input type="password" placeholder="관리자 비밀번호" value={pwd}
          ...
          style={{background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.14)',color:'white',marginBottom:10}}/>
        {error&&<p style={{color:'#FF7070',fontSize:12,marginBottom:10}}>{error}</p>}
        <button className="btn-gold" onClick={onLogin} style={{width:'100%',marginBottom:10}}>입장하기</button>
        <button className="btn-ghost" onClick={onBack}
          style={{color:'rgba(255,255,255,.3)',borderColor:'rgba(255,255,255,.1)',width:'100%'}}>돌아가기</button>
      </div>
    </div>
  );
}
    ...
    <div>
      {data.slice(0,8).map(({label,count})=>(
        <div key={label} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
          <span style={{fontSize:12,color:'rgba(255,255,255,.6)',minWidth:150,maxWidth:150,lineHeight:1.3,flexShrink:0}}>{label}</span>
          <div style={{flex:1,height:7,background:'rgba(255,255,255,.1)',borderRadius:4,overflow:'hidden'}}>
            <div style={{height:'100%',background:'#C9A84C',borderRadius:4,width:`${(count/max)*100}%`,transition:'width 1s ease'}}/>
          </div>
          <span style={{fontSize:13,fontWeight:700,color:'#C9A84C',minWidth:20}}>{count}</span>
          <span style={{fontSize:11,color:'rgba(255,255,255,.3)',minWidth:30}}>{total>0?Math.round(count/total*100):0}%</span>
        </div>
      ))}
    </div>
  );
    ...
    <div style={{minHeight:'100vh',background:'#0A1628'}}>
      ...
      <div style={{background:'rgba(255,255,255,.04)',borderBottom:'1px solid rgba(255,255,255,.07)',
        ...
        <div style={{maxWidth:1060,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <div style={{width:26,height:26,borderRadius:6,background:'linear-gradient(135deg,#C9A84C,#F0D98A)',
              display:'flex',alignItems:'center',justifyContent:'center'}}>
              <span style={{color:'#1B3A6B',fontWeight:900,fontSize:11}}>A</span>
            </div>
          ...
          <div style={{display:'flex',gap:8}}>
            <button onClick={onRefresh} className="btn-ghost" style={{color:'rgba(255,255,255,.45)',borderColor:'rgba(255,255,255,.12)',fontSize:12}}>🔄 새로고침</button>
            <button onClick={onBack} className="btn-ghost" style={{color:'rgba(255,255,255,.45)',borderColor:'rgba(255,255,255,.12)',fontSize:12}}>← 나가기</button>
          </div>
        </div>
      </div>
      <div style={{maxWidth:1060,margin:'0 auto',padding:'24px 20px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}}>
          {[
            ...
            <div key={i} style={{background:'rgba(255,255,255,.05)',borderRadius:16,padding:18,border:'1px solid rgba(255,255,255,.07)'}}>
        ...
        <div style={{display:'flex',gap:3,marginBottom:20,background:'rgba(255,255,255,.04)',padding:4,borderRadius:12,width:'fit-content'}}>
          {[['overview','📊 개요'],['details','📋 상세 응답'],['ai','🤖 AI 분석']].map(([t,l])=>(
            <button key={t} onClick={()=>setTab(t)} style={{padding:'7px 16px',borderRadius:8,border:'none',cursor:'pointer',
              background:tab===t?'#C9A84C':'transparent',color:tab===t?'#1B3A6B':'rgba(255,255,255,.45)',
              fontFamily:"'Noto Sans KR',sans-serif",fontWeight:tab===t?700:400,fontSize:13}}>{l}</button>
          ))}
        </div>
        {tab==='overview'&&(
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:18}}>
            {[
              {title:'🏢 부서별 응답',data:deptC},{title:'🤖 사용 중인 AI 도구',data:toolC},
              {title:'📈 AI 활용 수준',data:levelC},{title:'✨ AI에게 바라는 것',data:wishC},
              {title:'⚠️ 주요 우려사항',data:concC},{title:'📚 교육 수요',data:eduC},
            ].map(({title,data})=>(
              <div key={title} className="card-dark">
                <h3 style={{color:'white',fontSize:13,fontWeight:700,marginBottom:16}}>{title}</h3>
                {total===0?<p style={{color:'rgba(255,255,255,.25)',fontSize:13}}>아직 응답이 없습니다.</p>:<BarChart data={data} total={total}/>}
              </div>
            ))}
          </div>
        )}
        {tab==='details'&&(
          <div>
            {total===0
              ?<div style={{textAlign:'center',padding:'56px 20px',color:'rgba(255,255,255,.25)',fontSize:14,lineHeight:2}}>아직 응답이 없습니다.</div>
              :responses.map((r,i)=>(
                <div key={r.id} className="card-dark" style={{cursor:'pointer'}} onClick={()=>setExpanded(expanded===i?null:i)}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <span style={{background:'#C9A84C',color:'#1B3A6B',fontWeight:700,fontSize:11,padding:'3px 10px',borderRadius:20}}>{r.dept||'미기재'}</span>
                      <span style={{color:'white',fontSize:14,fontWeight:600}}>{r.name||'익명'}</span>
                      {r.position&&<span style={{color:'rgba(255,255,255,.35)',fontSize:12}}>· {r.position}</span>}
                    </div>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  {expanded===i&&(
                    <div style={{marginTop:16,paddingTop:16,borderTop:'1px solid rgba(255,255,255,.08)'}}>
                      {[
                        {l:'소속 부서',v:r.dept||'-'},{l:'성명',v:r.name||'-'},{l:'직책',v:r.position||'-'},
                        {l:'이메일',v:r.email||'-'},{l:'AI 도구',v:r.q1?.join(', ')||'-'},
                        {l:'활용 용도',v:r.q2?.join(', ')||'-'},{l:'AI 수준',v:r.q3||'-'},
                        {l:'TOP3 자동화',v:r.q5?.filter(Boolean).join(' | ')||'-'},
                        {l:'시간 빼앗는 업무',v:r.q6||'-'},{l:'AI에 바라는 것',v:r.q7?.join(', ')||'-'},
                        {l:'우려사항',v:r.q8?.join(', ')||'-'},{l:'교육 수준',v:r.q9||'-'},
                        {l:'자유 의견',v:r.q11||'-'},
                      ].map(({l,v})=>(
        ...
        {tab==='ai'&&(
          <div>
            <div className="card-dark">
              <h3 style={{color:'white',fontSize:15,fontWeight:700,marginBottom:8}}>🤖 Claude AI 자동 분석</h3>
              <p style={{color:'rgba(255,255,255,.45)',fontSize:13,marginBottom:20,lineHeight:1.7}}>
                수집된 {total}개 응답을 Claude AI가 분석하여<br/>부서별 Pain Point, 도입 우선순위, 교육 수요를 자동으로 정리합니다.
              </p>
              <button className="btn-gold" onClick={onAnalyze} disabled={total===0||isAnalyzing}>
                {isAnalyzing
                  ?<span style={{display:'flex',alignItems:'center',gap:8}}>
                    <span style={{width:16,height:16,border:'2px solid rgba(27,58,107,.3)',borderTop:'2px solid #1B3A6B',borderRadius:'50%',animation:'spin .8s linear infinite',display:'inline-block'}}/>
                    분석 중...
                  </span>
                  :`${total}개 응답 AI 분석 시작`
                }
            ...
            {aiAnalysis&&(
              <div style={{background:'rgba(201,168,76,.07)',borderRadius:18,padding:26,border:'1px solid rgba(201,168,76,.2)'}}>
                <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:18,paddingBottom:14,borderBottom:'1px solid rgba(201,168,76,.18)'}}>
                  <span style={{fontSize:18}}>✨</span>
                  <span style={{color:'#C9A84C',fontWeight:700,fontSize:14}}>AI 분석 결과</span>
                </div>
                <div style={{color:'rgba(255,255,255,.8)',fontSize:14,lineHeight:1.9,whiteSpace:'pre-wrap'}}>{aiAnalysis}</div>
              </div>
            )}
          </div>
        )}
...
export default function App() {
  ...
  // ── 라우팅 ────────────────────────────────────────────────────────────────
  if(view==='landing')
    return <LandingView onStart={()=>{setStep(1);setView('form');}} onAdmin={()=>setView('adminLogin')}/>;
  ...
  if(view==='thanks')
    return <ThanksView onNew={()=>{setForm(emptyForm());setStep(1);setView('landing');}}/>;

  if(view==='adminLogin')
    return <AdminLoginView pwd={adminPwd} setPwd={setAdminPwd}
              onLogin={handleAdminLogin} error={adminErr}
              onBack={()=>setView('landing')}/>;

  if(view==='admin')
    return <AdminView responses={allResp}
